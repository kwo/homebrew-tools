class OpencodeWeb < Formula
  desc "Homebrew service wrapper for running opencode web"
  ...
  def default_env
    <<~EOS
      # Environment for #{name}. Loaded by #{opt_bin}/opencode-web
      OPENCODE_PORT=4096
  ...
  def install
    (libexec/"opencode-web").write <<~BASH
      #!/bin/bash
      set -euo pipefail
    ...
    chmod 0755, libexec/"opencode-web"
    bin.install_symlink libexec/"opencode-web"
    (pkgshare/"opencode-web.env.example").write default_env
  end
  ...
  service do
    run [opt_bin/"opencode-web"]
    keep_alive true
    log_path var/"log/opencode-web.log"
    error_log_path var/"log/opencode-web.err.log"
  end

  def caveats
    <<~EOS
      Configure #{etc}/opencode-web.env, then start the service:
      ...
      Logs:
        ...
        tail -f #{var}/log/opencode-web.err.log
    EOS
  end

  test do
    output = shell_output("#{bin}/opencode-web --print-cmd")
